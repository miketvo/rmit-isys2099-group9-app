...
database.getBuyer = (username) => {
    return new Promise((resolve, reject)=>{
        db.poolWHAdmin.query(`SELECT * FROM buyer WHERE username = ?`, [username], (err, results) => {
            if (err) {
              console.error("error: " + err.stack);
              reject(err);
              return;
...
database.insertBuyer = (username) => {
    return new Promise((resolve, reject)=>{
        db.poolWHAdmin.query(`INSERT INTO buyer (username) VALUES (?)`, [username], (err, results) => {
            if (err) {
                console.error("error: " + err.stack);
                reject(err);
                return;
...
database.getSeller = (username) => {
    return new Promise((resolve, reject)=>{
        db.poolWHAdmin.query(`SELECT * FROM seller WHERE username = ?`, [username], (err, results) => {
            if (err) {
              console.error("error: " + err.stack);
              reject(err);
              return;
...
database.insertSeller = (username, shop_name) => {
    return new Promise((resolve, reject)=>{
        db.poolWHAdmin.query(`INSERT INTO seller (username, shop_name) VALUES (?, ?)`, [username, shop_name], (err, results) => {
            if (err) {
                console.error("error: " + err.stack);
                reject(err);
                return;
