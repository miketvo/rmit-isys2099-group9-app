...
const userController = {
  ...
  updateBuyerUsername: async (req, res) => {
    try {
      ...
      await db.poolBuyer.query(
        `UPDATE buyer SET username = ? WHERE username = ?`,
        [newUsername, oldUsername],
      );
      res.json({
        message: `Buyer ${newUsername} updated successfully`,
  ...
  updateWHAdminUsername: async (req, res) => {
    try {
      ...
      await db.poolWHAdmin.query(
        `UPDATE wh_admin SET username = ? WHERE username = ?`,
        [newUsername, oldUsername],
      );
      res.json({
        message: `Admin ${newUsername} updated successfully`,
        username: newUsername,
        role: "admin",
      });
    } catch (error) {
    ...
    try {
      ...
      if (newUsername && newShopName) {
        await db.poolSeller.query(
          `UPDATE seller SET username = ?, shop_name = ? WHERE username = ?`,
          [newUsername, newShopName, oldUsername],
        );
        res.json({
          message: `Seller ${newUsername} updated successfully`,
          username: newUsername,
        ...
        await db.poolSeller.query(
          `UPDATE seller SET username = ? WHERE username = ?`,
          [newUsername, oldUsername],
        );
        res.json({
          message: `Seller ${newUsername} updated successfully`,
        ...
        await db.poolSeller.query(
          `UPDATE seller SET shop_name = ? WHERE username = ?`,
          [newShopName, oldUsername],
        );
        res.json({
          message: `Seller ${newUsername} updated successfully`,
  ...
  deleteBuyerByUsername: async (req, res) => {
    try {
      ...
      await db.poolBuyer.query(`DELETE FROM buyer WHERE username = ?`, [
        username,
      ]);
      res.json({
        message: `Buyer ${username} deleted successfully`,
        username: username,
        role: "buyer",
      });
    } catch (error) {
  ...
  deleteWHAdminByUsername: async (req, res) => {
    try {
      const username = req.username;
      await db.poolWHAdmin.query(`DELETE FROM wh_admin WHERE username = ?`, [
        username,
      ]);
      res.json({
        message: `WH Admin ${username} deleted successfully`,
        username: username,
        role: "admin",
      });
    } catch (error) {
  ...
  deleteSellerByUsername: async (req, res) => {
    try {
      const username = req.username;
      await db.poolSeller.query(`DELETE FROM seller WHERE username = ?`, [
        username,
      ]);
      res.json({
        message: `Seller ${username} deleted successfully`,
        username: username,
        role: "seller",
      });
    } catch (error) {
