  ...
  useEffect(() => {
    if (edited) {
      setInboundOrderData(preData => ({
        ...preData,
        quantity: editedInboundOrderData.quantity
          ? editedInboundOrderData.quantity
          : "",
        product_id: editedInboundOrderData.product_id
          ? editedInboundOrderData.product_id
          : "",
      }));
    }
  }, [
      ...
      if (inboundOrderData.quantity > 0) {
        if (created) {
          ...
          const response = await putDataAPI(
            `inbound-order/update/${editedInboundOrderData.id}`,
            inboundOrderData,
          );
