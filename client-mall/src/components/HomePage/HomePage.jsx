...
const HomePage = () => {
  ...
  useEffect(() => {
    if (sellerFiltered.length > 0) {
      ...
      const filtered = clonedData.filter(item =>
        sellerFiltered.includes(item.seller),
      );
    ...
    if (categoryFiltered.length > 0) {
      const clonedData = [...products];
      const filtered = clonedData.filter(item =>
        categoryFiltered.includes(item.category),
      );
    ...
    if (sellerFiltered.length > 0 && categoryFiltered.length > 0) {
      const clonedData = [...products];
      const filterSeller = clonedData.filter(item =>
        sellerFiltered.includes(item.seller),
      );
      const filterCategory = filterSeller.filter(item =>
        categoryFiltered.includes(item.category),
      );
  ...
  return (
    <section
      className="main_page mt-5"
      style={{ minHeight: "calc(100vh - var(--horizontalNavBarHeight))" }}
    >
      <div className="container d-flex column-gap-3 gap-0">
        <div className="position-relative z-3 col-2 px-2 py-4 bg-light h-100">
          <div className="mb-4">
            <SellerFiltered
              compData={{ products, sellerFiltered }}
              compFunction={{ setSellerFiltered }}
            />
          </div>
