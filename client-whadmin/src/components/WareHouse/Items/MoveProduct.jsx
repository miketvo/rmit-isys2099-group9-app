...
const MoveProduct = ({ compData }) => {
  ...
  const sellersOnly =
    productID !== "" && category !== ""
      ? productsData
          .filter(
            item =>
              item.id === parseInt(productID, 10) && item.category === category,
          )
          .map(item => item.seller)
      : productID !== ""
      ? productsData
          .filter(item => item.id === parseInt(productID, 10))
          .map(item => item.seller)
      : category !== ""
      ? productsData
  ...
  const categoryOnly =
    productID !== "" && seller !== ""
      ? productsData
          .filter(
            item =>
              item.seller === seller && item.id === parseInt(productID, 10),
          )
  ...
  const productOnly =
    category !== "" && seller !== ""
      ? productsData
          .filter(item => item.seller === seller && item.category === category)
  ...
  useEffect(() => {
    ...
    if (seller && productID && category) {
      ...
      const currentProductWarehouse = stockpileData.filter(
        item => item.product_id === productResultId,
      );
      ...
      const getCurrentQuantityProductInWarehouse = currentProductWarehouse.find(
        item => item.warehouse_id === parseInt(warehouseId, 10),
      );
  ...
  const wareHouseFilter = warehouseId
    ? wareHouseData.filter(item => item.id !== parseInt(warehouseId, 10))
    : [];
    ...
    <div className="move_product mt-3">
      <div className="container_fluid">
        <div className="row g-2">
          <OptionComponent
            name="seller"
            compData={sellersOnly}
            compState={seller}
            compFunction={setSeller}
          />
          <OptionComponent
            name="category"
            compData={categoryOnly}
            compState={category}
            compFunction={setCategory}
          />
          <OptionComponent
            name="product"
            compData={productOnly}
            compState={productID}
            compFunction={setProductID}
          />
        </div>
        <div className="mt-4">
          <div className="d-flex">
            <div className="col-5">
              ...
              >
                ...
                {wareHouseFounded &&
                  wareHouseFounded.map(warehouse => (
                  ...
                  item =>
                    item.id === parseInt(warehouseId, 10) && (
                      <ul key={item.id}>
                        ...
                        <li>
                          <span className="fw-bold">
                            Available Volume: {item.available_volume}
                          </span>
                        </li>
                        <li>
                        ...
                        <li>
                          <span>Street Number: {item.street_number}</span>
                        </li>
                        <li>
