...
const ProductAttributes = ({ compData, compFunction }) => {
  const { created, edited, productCategoryData, editedProductAttributeData } =
    compData;
  ...
  const [productAttributeData, setProductAttributeData] =
    useState(AttributeState);
  const [categoryList, setCategoryList] = useState([]);
  const { attribute_name, attribute_type, required, category } =
  ...
  useEffect(() => {
    if (edited) {
      setProductAttributeData(preData => ({
        ...preData,
        attribute_name: editedProductAttributeData.attribute_name
          ? editedProductAttributeData.attribute_name
          : "",
        attribute_type: editedProductAttributeData.attribute_type
          ? editedProductAttributeData.attribute_type
          : "",
        required: editedProductAttributeData.required
          ? editedProductAttributeData.required
          : false,
      }));

      setCategoryList(
        editedProductAttributeData.categories
          ? editedProductAttributeData.categories
          : [],
      );
    }
  ...
  const handleChangeInput = e => {
    ...
    const parseValue =
      name === "required" ? (value === "true" ? true : false) : value;
    ...
    try {
      if (created) {
        ...
        const response = await putDataAPI(
          `attribute/update/${editedProductAttributeData.attribute_name}`,
          {
        ...
        if (response.status === 200 || response.status === 201) {
          setProductAttributesData(preData => {
            ...
            const matchingObjects = clonedData.filter(
              item => item.attribute_name === editedData.attribute_name,
            );
          ...
          toast.success(
            `Edit Attribute "${editedProductAttributeData.attribute_name}" with ${category} Successfully`,
          );
        }
      }
    } catch (error) {
                  ...
                  {categoryNameArray?.map(item => (
                    <div className="form-check" key={item}>
                      <input
                        ...
                        id="flexCheckDefault"
                        value={item}
                        checked={categoryList.includes(item) ? true : false}
