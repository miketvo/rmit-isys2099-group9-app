...
const WareHouseComponent = () => {
    ...
    const fetchProductAttributesData = async () => {
      try {
        ...
        setProductAttributesData(
          result.data.map(item => ({ ...item, required: item.required === 1 })),
        );
      } catch (error) {
        // Handle the error
        console.error("Error fetching data:", error);
  ...
  const handleOpenEditedMode = ({
    ...
    if (place === "warehouse") {
      ...
      setEditedProductCategoryData(
        productCategoryData.find(item => item.category_name === category_name),
      );
      setPopUpState(prevState => ({
        ...prevState,
        state: !prevState.state,
    ...
    try {
      if (place === "warehouse") {
        ...
        const response = await deleteDataAPI(
          `product-category/delete/${category_name}`,
        );
        if (response.status === 200 || response.status === 201) {
          setProductCategoryData(preState => [
            ...preState.filter(item => item.category_name !== category_name),
          ]);

          toast.success(
            `Delete Category with name ${category_name} Successfully`,
          );
        }
      } else if (place === "product_attribute") {
        const response = await deleteDataAPI(
          `attribute/delete/${attribute_name}`,
        );
        if (response.status === 200 || response.status === 201) {
          setProductAttributesData(preState => [
            ...preState.filter(item => item.attribute_name !== attribute_name),
          ]);
  ...
  const WareHouseTabsMap = [
    ...
    },
    {
      ...
      component: (
        <Categories
          compData={productCategoryData}
          compFunction={ProductCategoryFunction}
        />
      ),
      created: true,
    },
    {
      ...
      component: (
        <ProductAttributes
          compData={productAttributesData}
          compFunction={ProductAttributeFunction}
        />
      ),
      created: true,
    },
